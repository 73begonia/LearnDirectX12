...
class transformation : public D3DApp
{
public:
	transformation(HINSTANCE hInstance);
...
private:
	...
	std::unique_ptr<UploadBuffer<TransformationConstants>> mTransformationCB = nullptr;
	...
	PSTR cmdLine, int showCmd)
{
	...
	try
	{
		transformation theApp(hInstance);
...
transformation::transformation(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
}
...
void transformation::OnResize()
{
	D3DApp::OnResize();
}

void transformation::Update(const GameTimer& gt)
{
	XMMATRIX transform = XMMatrixIdentity();

	transform = XMMatrixTranslation(0.5f, -0.5f, 0.0f) * transform;

	float angle = static_cast<float>(gt.TotalTime());
	transform = XMMatrixRotationZ(angle) * transform;

	TransformationConstants constants;
	XMStoreFloat4x4(&constants.transform, XMMatrixTranspose(transform));
	mTransformationCB->CopyData(0, constants);
}

void transformation::Draw(const GameTimer& gt)
{
	...
	ID3D12DescriptorHeap* ppDescriptorHeaps[] = { mSrvDescriptorHeap.Get(), mCbvHeap.Get() };
	mCommandList->SetDescriptorHeaps(1, ppDescriptorHeaps);
	...
	mCommandList->SetGraphicsRootDescriptorTable(1, cbv);
...
void transformation::LoadTextures()
{
...
void transformation::BuildConstantBuffers()
{
	mTransformationCB = std::make_unique<UploadBuffer<TransformationConstants>>(md3dDevice.Get(), 1, true);

	UINT cbByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(TransformationConstants));

	D3D12_GPU_VIRTUAL_ADDRESS cbAddress = mTransformationCB->Resource()->GetGPUVirtualAddress();
	
	D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc;
	cbvDesc.BufferLocation = cbAddress;
	cbvDesc.SizeInBytes = d3dUtil::CalcConstantBufferByteSize(sizeof(TransformationConstants));
...
void transformation::BuildRootSignature()
{
...
void transformation::BuildDescriptorHeaps()
{
...
void transformation::BuildShadersAndInputLayout()
{
	HRESULT hr = S_OK;

	mvsByteCode = d3dUtil::CompileShader(L"shaders\\5.1.transform.hlsl", nullptr, "VS", "vs_5_0");
	mpsByteCode = d3dUtil::CompileShader(L"shaders\\5.1.transform.hlsl", nullptr, "PS", "ps_5_0");
...
void transformation::BuildTriangleGeometry()
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> transformation::GetStaticSamplers()
{
